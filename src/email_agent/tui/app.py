...
class EmailList(DataTable):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.add_columns("Priority", "Subject", "From", "Date", "Category", "Score", "Status")
    ...
    def load_emails(self, emails: List[Email], show_triage_data: bool = False):
        ...
        for email in emails:
            ...
            elif show_triage_data and hasattr(email, 'connector_data') and email.connector_data:
                attention_score = f"{email.connector_data.get('attention_score', 0):.2f}"
...
class DraftSuggestionsPanel(Vertical):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Button("🔍 Analyze Writing Style", id="analyze-style-btn", variant="primary")
        yield Label("")
    ...
    def update_current_email(self, email: Optional[Email]):
        ...
        if email:
            self.query_one("#current-email-subject", Label).update(email.subject[:50] + "...")
        else:
...
class FeedbackDialog(ModalScreen):
    ...
    def compose(self) -> ComposeResult:
        with Container(id="feedback-dialog"):
            ...
            with Horizontal():
                yield Button("✅ Submit Feedback", id="submit-feedback-btn", variant="success")
...
class LearningInsightsDialog(ModalScreen):
    ...
    def compose(self) -> ComposeResult:
        with Container(id="insights-dialog"):
            ...
            yield Label(f"📊 Total feedback received: {learning_stats.get('total_feedback_received', 0)}")
            yield Label(f"🎯 Learning active: {'✅ Yes' if learning_stats.get('learning_active') else '❌ No'}")
            yield Label("")
            ...
            yield Label("")
            with Horizontal():
                yield Button("📋 Full Report", id="full-insights-btn", variant="primary")
...
class SenderScoresDialog(ModalScreen):
    ...
    def compose(self) -> ComposeResult:
        with Container(id="scores-dialog"):
            ...
            sorted_senders = sorted(self.sender_scores.items(), key=lambda x: x[1], reverse=True)
            ...
            for sender, score in sorted_senders[:20]:  # Top 20
                ...
                scores_table.add_row(sender[:30] + "..." if len(sender) > 30 else sender, 
                                   f"{score:.3f}", priority_level)
...
class NarrativeBriefPanel(Vertical):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Horizontal():
            ...
            yield Button("📖 Generate Narrative", id="generate-narrative-btn", variant="success")
    ...
    def show_narrative_brief(self, brief_data: dict):
        ...
        log.write(f"[dim]📖 Reading time: {reading_time}s | Narrative score: {narrative_score:.1f}/1.0[/dim]")
        log.write("")
...
class StatsSidebar(Vertical):
    ...
    def update_triage_stats(self, triage_stats: dict):
        """Update triage statistics display."""
        self.query_one("#triage-priority", Label).update(f"Priority: {triage_stats.get('priority_count', 0)}")
        self.query_one("#triage-regular", Label).update(f"Regular: {triage_stats.get('regular_count', 0)}")
        self.query_one("#triage-archived", Label).update(f"Archived: {triage_stats.get('archived_count', 0)}")
...
"""
    ...
    async def action_smart_triage(self) -> None:
        ...
        try:
            ...
            emails = self.db.get_emails(limit=100, since=datetime.now() - timedelta(days=7))
            ...
            try:
                ...
                smart_inbox_view.update_triage_stats(smart_inbox_results.get("stats", {}))
    ...
    async def action_show_priority_inbox(self) -> None:
        ...
        try:
            ...
            emails = self.db.get_emails(limit=50, since=datetime.now() - timedelta(days=7))
            smart_inbox_results = await self.crew.execute_task("smart_inbox", emails=emails, rules=[])
    ...
    async def action_show_archived_emails(self) -> None:
        ...
        try:
            ...
            self.notify(f"Showing {len(archived_emails)} auto-archived emails", timeout=3)
    ...
    async def action_analyze_writing_style(self) -> None:
        ...
        try:
            ...
            self.notify(f"Writing style analyzed! Formality: {formality:.2f}, Avg length: {avg_length} words", timeout=5)
    ...
    async def action_generate_narrative_brief(self) -> None:
        ...
        try:
            ...
            reading_time = brief_results.get("brief", {}).get("estimated_reading_time", 45)
            self.notify(f"Narrative brief generated! Reading time: {reading_time}s", timeout=5)
            
        except Exception as e:
            self.notify(f"Narrative brief generation failed: {str(e)}", severity="error")
    ...
    async def action_show_full_insights(self) -> None:
        ...
        try:
            ...
            log_display.write("[bold cyan]🧠 Comprehensive Learning Insights[/bold cyan]")
            ...
            if category_insights:
                ...
                for category, prefs in category_insights.items():
                    ...
                    log_display.write(f"  - Archive tendency: {prefs.get('archive_tendency', 0.0):.2f}")
                    log_display.write(f"  - Feedback count: {prefs.get('feedback_count', 0)}")
            ...
            if time_insights:
                ...
                if priority_hours:
                    sorted_hours = sorted(priority_hours.items(), key=lambda x: x[1], reverse=True)
                    log_display.write("Peak priority hours:")
                    for hour, count in sorted_hours[:5]:
                        log_display.write(f"  • {hour:02d}:00 - {count} priority emails")
