...
class ActionExtractorAgent:
    ...
    async def extract_actions(self, email: Email) -> Dict[str, Any]:
        ...
        try:
            response = await self.client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "You are an expert email analyst. Extract actionable information accurately. Return only valid JSON."},
    ...
    async def track_commitments(self, email: Email, actions: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    async def generate_action_summary(self, actions_list: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Generate a summary of all actions across emails."""
        
        total_actions = len(actions_list)
        urgent_actions = sum(1 for actions in actions_list 
                           for item in actions.get("action_items", []) 
                           if item.get("priority") == "high")
        
        needs_response = sum(1 for actions in actions_list 
                           if actions.get("needs_response", False))
