...
""")
        ...
        try:
            response = await self.client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "You are an expert email thread analyst. Extract key insights and actionable information from email conversations. Return only valid JSON."},
    ...
    async def get_thread_insights(self, thread_summary: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            response = await self.client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "You are a business communication analyst. Provide strategic insights on email thread effectiveness and outcomes."},
    ...
    async def summarize_multiple_threads(self, thread_groups: List[List[Email]]) -> List[Dict[str, Any]]:
    ...
    async def generate_threads_overview(self, thread_summaries: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        urgent_threads = len([s for s in valid_summaries if s.get('priority_level') == 'urgent'])
        unresolved_threads = len([s for s in valid_summaries if s.get('thread_status') in ['ongoing', 'stalled']])
        requires_attention = len([s for s in valid_summaries if s.get('requires_attention', False)])
        ...
        return {
            ...
            "efficiency_metrics": {
                "avg_efficiency": sum(s.get('insights', {}).get('efficiency_score', 5) for s in valid_summaries) / total_threads if total_threads > 0 else 0,
                "avg_collaboration": sum(s.get('insights', {}).get('collaboration_score', 5) for s in valid_summaries) / total_threads if total_threads > 0 else 0
            },
            "top_priorities": [
                s for s in valid_summaries 
                if s.get('priority_level') in ['urgent', 'high'] and s.get('requires_attention', False)
            ][:5],
            "recommendations": [
