...
async def _generate_drafts(email_id: str, num_suggestions: int, context: str):
    ...
    try:
        ...
        console.print(f"[blue]Generating {num_suggestions} draft suggestions for:[/blue]")
        ...
        for i, suggestion in enumerate(suggestions):
            ...
            suggestion_content.append("")
            suggestion_content.append(f"[dim]Confidence: {suggestion['confidence']:.2f} | Style Match: {suggestion['style_match']:.2f}[/dim]")
            suggestion_content.append(f"[dim]Tone: {suggestion['suggested_tone']} | Length: {suggestion['estimated_length']}[/dim]")
            suggestion_content.append(f"[dim]Reasoning: {suggestion['reasoning']}[/dim]")
        ...
        # Show usage tip
        console.print(f"[dim]Tip: Use 'email-agent drafts use-draft --email-id {email_id} --draft-index <0-{len(suggestions)-1}>' to use a suggestion[/dim]")
...
async def _analyze_writing_style(force_refresh: bool, min_emails: int):
    ...
    try:
        ...
        console.print("[blue]Retrieving sent emails for writing style analysis...[/blue]")
        
        # Get sent emails (this would need to be implemented in DatabaseManager)
        # For now, we'll simulate getting sent emails
        sent_emails = db.get_sent_emails(limit=100)  # This method needs to be implemented
        
        if len(sent_emails) < min_emails:
            console.print(f"[yellow]Only {len(sent_emails)} sent emails found. Need at least {min_emails} for accurate analysis.[/yellow]")
            if len(sent_emails) == 0:
                console.print("[red]No sent emails found. Make sure you have sent emails synced.[/red]")
                return
        
        console.print(f"[green]Found {len(sent_emails)} sent emails for analysis[/green]")
        ...
        table.add_row("Average Length", f"{results['avg_length']} words", "Typical email length")
        table.add_row("Formality Score", f"{results['formality_score']:.2f}/1.0", "0=casual, 1=formal")
        table.add_row("Greeting Style", results['greeting_style'], "Most common greeting")
        table.add_row("Closing Style", results['closing_style'], "Most common closing")
        table.add_row("Sentence Complexity", f"{results['sentence_complexity']:.2f}/1.0", "0=simple, 1=complex")
        ...
        console.print("[green]Writing style analysis saved. Draft suggestions will now be personalized![/green]")
...
async def _show_style_summary():
    ...
    try:
        ...
        if summary.get("status") == "not_analyzed":
            ...
            console.print("Run 'email-agent drafts analyze-style' to analyze your writing style.")
            return
        ...
        table.add_row("Sentence Complexity", f"{summary.get('sentence_complexity', 0):.2f}/1.0")
...
async def _use_draft_suggestion(email_id: str, draft_index: int):
    ...
    try:
        ...
        if draft_index >= len(suggestions):
            console.print(f"[red]Draft index {draft_index} not available. Only {len(suggestions)} suggestions generated.[/red]")
            return
        ...
        try:
            ...
            console.print("[dim]Install 'pyperclip' to enable clipboard functionality[/dim]")
        except Exception:
        ...
        console.print("[dim]To save as draft in your email client, copy the content above.[/dim]")
