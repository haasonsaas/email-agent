    ...
    async def run_auto_handler():
        ...
        with db.get_session() as session:
            ...
            if not unread_emails:
                console.print("[green]✅ All caught up! No emails need handling.[/green]")
                return
        ...
        if dry_run:
            ...
            with db.get_session() as session:
                ...
                for email_data in triage_results.get('priority_inbox', []):
                    email_orm = session.query(EmailORM).filter_by(id=email_data['id']).first()
                    if email_orm:
...
def smart_actions(
    ...
    async def run_smart_actions():
        ...
        with db.get_session() as session:
            from ..storage.models import EmailORM
            recent_emails = session.query(EmailORM).filter(
                ~EmailORM.tags.like('%action_processed%')
            ).order_by(EmailORM.received_date.desc()).limit(limit).all()
            
            console.print(f"Found [yellow]{len(recent_emails)}[/yellow] emails to analyze for actions")
        ...
        for i, (email, actions) in enumerate(zip(emails, actions_results)):
            if 'error' in actions:
                console.print(f"[red]❌ Error processing {email.subject[:40]}...: {actions['error']}[/red]")
                continue
            ...
            deadlines_count = sum(1 for item in actions.get('action_items', []) if item.get('deadline'))
            ...
            if actions_count > 0 or commitments_count > 0 or meetings_count > 0:
                ...
                if actions.get('needs_response'):
                    ...
                    urgency_color = 'red' if urgency == 'urgent' else 'yellow' if urgency == 'normal' else 'green'
                    console.print(f"   📢 Needs response: [{urgency_color}]{urgency}[/{urgency_color}]")
                
                if actions_count > 0:
                    console.print(f"   📋 Actions: {actions_count}")
                    for action in actions.get('action_items', [])[:2]:
                        deadline_str = f" (Due: {action['deadline']})" if action.get('deadline') else ""
                        console.print(f"     • {action['action'][:60]}...{deadline_str}")
                ...
                if gmail_service and not dry_run:
                    try:
                        if apply_labels:
                            await gmail_service.apply_action_labels(email.message_id, actions)
                            console.print("     🏷️  Gmail labels applied")
                        
                        if generate_replies and actions.get('needs_response'):
                            reply = await gmail_service.generate_smart_reply(email, actions)
                            if reply:
                                console.print(f"     💬 Smart reply generated ({len(reply)} chars)")
        ...
        if summary['deadlines_this_week'] > 0:
            console.print(f"[yellow]📅 {summary['deadlines_this_week']} items due this week[/yellow]")
...
@app.command()
def thread_summary(
    thread_id: Optional[str] = typer.Option(None, help="Specific thread ID to summarize"),
    ...
    insights: bool = typer.Option(False, "--insights", help="Generate deeper insights for threads"),
    overview: bool = typer.Option(False, "--overview", help="Generate overview of all thread summaries")
):
    ...
    async def run_thread_summary():
        ...
        with db.get_session() as session:
            ...
            if thread_id:
                ...
                thread_emails = session.query(EmailORM).filter(
                    EmailORM.thread_id == thread_id
                ).order_by(EmailORM.date).all()
                
                if not thread_emails:
                    console.print(f"[red]❌ No emails found for thread ID: {thread_id}[/red]")
                    return
                
                thread_groups = [thread_emails]
                console.print(f"Found thread with [yellow]{len(thread_emails)}[/yellow] emails")
                ...
                ).order_by(EmailORM.received_date.desc()).all()
                ...
                thread_groups.sort(key=lambda emails: max(e.received_date for e in emails), reverse=True)
                thread_groups = thread_groups[:limit]
                
                console.print(f"Found [yellow]{len(thread_groups)}[/yellow] multi-email threads to summarize")
                
                if not thread_groups:
                    console.print("[green]✅ No multi-email threads found to summarize.[/green]")
                    return
        ...
        for i, (emails, summary) in enumerate(zip(email_thread_groups, summaries)):
            if 'error' in summary:
                console.print(f"[red]❌ Error summarizing thread {i+1}: {summary['error']}[/red]")
                continue
            
            console.print(f"\n🧵 [bold]Thread {i+1}:[/bold] {emails[0].subject[:60]}...")
            console.print(f"   📧 Emails: {summary['email_count']}")
            console.print(f"   📅 Date Range: {summary['date_range']['start'][:10]} to {summary['date_range']['end'][:10]}")
            ...
            if summary.get('action_items'):
                ...
                for action in summary['action_items'][:3]:
                    deadline_str = f" (Due: {action.get('deadline', 'No deadline')})" if action.get('deadline') else ""
                    owner_str = f" [{action.get('owner', 'Unassigned')}]" if action.get('owner') else ""
...
def feedback(
    ...
    agent_type: str = typer.Option("categorizer", help="Agent type (categorizer, prioritizer, action_extractor)"),
    decision_type: str = typer.Option("category", help="Decision type (category, priority, actions)"),
    feedback_text: str = typer.Option("", "--feedback", help="Your feedback on the AI decision"),
    correct_decision: str = typer.Option("", "--correct", help="What the correct decision should have been"),
    interactive: bool = typer.Option(False, "--interactive", "-i", help="Interactive feedback mode")
):
    ...
    async def run_feedback():
        ...
        if interactive:
            ...
            confidence = typer.prompt("How confident are you in this feedback? (0.0-1.0)", default="0.8", type=float)
        else:
            confidence = 0.8
...
def learning_stats():
    ...
    async def run_learning_stats():
        ...
        effectiveness_color = 'green' if effectiveness > 70 else 'yellow' if effectiveness > 50 else 'red'
        console.print(f"  Learning effectiveness: [{effectiveness_color}]{effectiveness:.1f}%[/{effectiveness_color}]")
        ...
        if stats['pattern_stats']:
            ...
            for pattern_type, pattern_info in stats['pattern_stats'].items():
                ...
                console.print(f"  {pattern_type}: {pattern_info['count']} patterns, [{color}]{success_rate:.1f}% success[/{color}]")
...
@app.command()
def commitments(
    show_overdue: bool = typer.Option(False, "--overdue", help="Show only overdue items"),
    days_ahead: int = typer.Option(30, help="Number of days ahead to look for commitments"),
    report: bool = typer.Option(False, "--report", help="Generate comprehensive commitment report")
):
    ...
    async def run_commitments():
        ...
        if report:
            ...
            report_data = await tracker.generate_commitment_report(days_ahead=days_ahead)
            ...
            console.print(f"  Overdue waiting items: [yellow]{summary['overdue_waiting_items']}[/yellow]")
            console.print(f"  Due today: [red]{summary['due_today']}[/red]")
            console.print(f"  Due this week: [yellow]{summary['due_this_week']}[/yellow]")
            
            # AI Insights
            if report_data.get('ai_insights'):
                insights = report_data['ai_insights']
                workload_color = {
                    'light': 'green', 'moderate': 'yellow', 
            ...
            if not overdue_items['overdue_commitments'] and not overdue_items['overdue_waiting']:
                console.print("[green]✅ No overdue items! You're all caught up.[/green]")
                return
            ...
            if overdue_items['overdue_commitments']:
                ...
                for item in overdue_items['overdue_commitments']:
                    console.print(f"  • {item['description'][:70]}...")
                    console.print(f"    To: {item.get('committed_to', 'Unknown')} | {item['days_overdue']} days overdue")
            
            if overdue_items['overdue_waiting']:
                console.print(f"\n⏰ [bold]Overdue Waiting Items ({len(overdue_items['overdue_waiting'])}):[/bold]")
                for item in overdue_items['overdue_waiting']:
                    console.print(f"  • {item['description'][:70]}...")
                    console.print(f"    From: {item.get('waiting_from', 'Unknown')} | {item['days_overdue']} days overdue")
            ...
            for commitment in pending[:15]:  # Limit display
                desc = commitment['description'][:70] + "..." if len(commitment['description']) > 70 else commitment['description']
                ...
                console.print(f"    To: {commitment.get('committed_to', 'Unknown')} | [{urgency_color}]{urgency_text}[/{urgency_color}] | {commitment['status']}")
...
def mark_complete(
    ...
    async def run_mark_complete():
        ...
        console.print(f"[cyan]Marking commitment {commitment_id} as completed...[/cyan]")
        ...
        if success:
            console.print(f"[green]✅ Commitment {commitment_id} marked as completed![/green]")
            if notes:
